...
class App extends Component {
  ...
  incrementTimeElapsedInSeconds = () => {
    const {secRem} = this.state
    const isTimerCompleted = secRem === 0
  ...
  getImages = () => {
    ...
    const requiredList = imagesList.filter(each => each.category === activeTab)
    return requiredList
  }

  clickandCheck = id => {
    const {randomImg} = this.state
    const findImg = imagesList.find(each => each.imageUrl === randomImg)

    if (findImg.id === id) {
      const x = Math.floor(Math.random() * imagesList.length)
      const nextImg = imagesList[x].imageUrl
      this.setState(prev => ({randomImg: nextImg, score: prev.score + 1}))
    } else {
      this.clearTimerInterval()
      this.setState({isTimerRunning: false})
    }
  ...
  renderBottom = () => {
    const {activeTab, randomImg} = this.state
    const filteredImages = this.getImages()

    return (
      <div className="bottom">
        <img className="mainimg" src={randomImg} alt="match" />
        <ul className="tabUnorder">
          {tabsList.map(each => (
            <TabItem
        ...
        <ul className="imageUnorder">
          {filteredImages.map(each => (
            <ImageItem
              clickandCheck={this.clickandCheck}
              imageDetails={each}
              key={each.id}
            />
          ))}
  ...
  renderScoreCard = () => {
    ...
    return (
      <div className="gameover">
        ...
        <p>YOUR SCORE</p>
        <p>{score}</p>
        <button className="resetbox" type="button" onClick={this.onReplay}>
          <div>
            <img
              alt="reset"
  ...
  render() {
    const {secRem, isTimerRunning, score} = this.state
    const display = isTimerRunning
      ? this.renderBottom()
    ...
    return (
      <div className="outside">
        <div className="inside">
          <NavBar secRem={secRem} score={score} />
          {display}
        </div>
      </div>
    )
